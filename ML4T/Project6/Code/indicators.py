...
def bollingerbands(df,lookback ,startdate , enddate):
...
def PriceSMA(df,lookback ,startdate , enddate):
    price = df['JPM']
    rolling_std = price.rolling(window=lookback, min_periods=lookback).std()
    sma = price.rolling(window=lookback, min_periods=lookback).mean()
...
def test_code():
    ...
    price,sma,top_band,bottom_band ,bbp = bollingerbands(df,lookback ,start_dt,end_dt)
    ...
    price , ema26 ,ema12,signal,macd,crossover = MACD(df2,start_dt,end_dt)
    ...
    price,sma,crossover = PriceSMA(df, lookback, start_dt, end_dt)
