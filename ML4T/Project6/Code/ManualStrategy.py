...
class ManualStrategy(object):
    ...
    def testPolicy(self, symbol="AAPL", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000):
        ...
        price1, sma1, top_band, bottom_band, bbp = bollingerbands(df,lookback,sd,ed)

        bbp1 = bbp.loc[sd:ed]

        #Calculate Price/SMA

        price2,sma2,crossover = PriceSMA(df,lookback,sd,ed)
        pricebysma = crossover.loc[sd:ed]

        #Calculate Macd_crossover
        price, ema26, ema12, signal, macd, crossover_macd = MACD(df,sd,ed)
